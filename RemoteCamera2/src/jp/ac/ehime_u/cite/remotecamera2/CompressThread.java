...
public class CompressThread extends Thread {
	...
	public void run(){
		...
		yuvimage.compressToJpeg(new Rect(0,0,width,height), CameraActivity.compress_ratio, byte_array_out_stream);
		if(byte_array_out_stream.size() <= 60000){
		// サービスを使用し、画像データの送信
		RINT rint = new RINT();
		byte flag = (byte) (rint.FLAG_PACKAGE_NAME | rint.FLAG_INTENT_ACTION);
		String package_name = context.getPackageName()+".ImageViewerActivity";
		String s_null = null;
		List<String> s_list = null;
		try {
			// Serviceを通して送信
			if(CameraActivity.sendManager != null){
				CameraActivity.sendManager.SendMessage(CameraActivity.calling_address, CameraActivity.my_address, flag, package_name, Intent.ACTION_VIEW, 0, s_null, s_null, s_list, byte_array_out_stream.toByteArray());
...
//
//			yuvimage.compressToJpeg(new Rect(0,0,width,height), CameraActivity.compress_ratio, f_out);
//
////			if(isYuv422())
