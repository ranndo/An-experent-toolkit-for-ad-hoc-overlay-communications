	...
	public static void process(byte[] receiveBuffer,byte[] preHopAddress, boolean bluetoothFlag, byte[] my_address, Context context, AODV_Service mAODV_Service){
		...
		synchronized (lock) {
			...
			case 1:	// RREQ
				...
				mAODV_Service.appendMessageOnActivity("A@ ");
				// ホップ数+1,
				receiveBuffer[3]++;
				...
				if(!RREQ.isToMe(receiveBuffer, my_address) || bload_cast_flag ){
					for(int k=0;k<4;k++)
						mAODV_Service.appendMessageOnActivity("\nD[0]@"+receiveBuffer[8+k]+":"+my_address[k]);
	    	...
	    	case 0: // 通信相手からのメッセージ
	    		...
	    		if( isToMe(receiveBuffer,my_address) || bload_cast_flag){
	    			...
	    			// 宛先までの有効経路を持っているか検索
	    			byte[] atesaki = getAddressDest(receiveBuffer);
					index = mAODV_Service.searchToAdd(atesaki);
			...
			case 2:	//RREPを受け取った場合
				...
				mAODV_Service.appendMessageOnActivity("RREP("+mesLength+"):getFlagA ="+String.valueOf(RRep.getFlagA(receiveBuffer, mesLength))+"\n");
			...
			case 5: // (データ付き)インテント発行メッセージを受け取った場合
				...
				if((RINT.FLAG_PACKAGE_NAME & rint.flag) != 0)
					mAODV_Service.writeLogData(53, AODV_Service.getStringByByteAddress(my_address), AODV_Service.getStringByByteAddress(rint.source_address),
							AODV_Service.getStringByByteAddress(rint.destination_address), rint.dataMap.size(), rint.package_name);
				else
					mAODV_Service.writeLogData(53, AODV_Service.getStringByByteAddress(my_address), AODV_Service.getStringByByteAddress(rint.source_address),
							AODV_Service.getStringByByteAddress(rint.destination_address), rint.dataMap.size(), null);
	    		if(to_me_message){
...
//					});
		            ...
		            if(rint.getData() != null){
		            	//Log.d("AODV_type5","D");
		            	intent.putExtra("HashMap", rint.getData());
		            }
